...
class PlayState extends MusicBeatState
{
	...
	function set_playbackRate(value:Float):Float
	{
		if(generatedMusic)
		{
			...
			if(splitVocals && opponentVocals != null) opponentVocals.pitch = value;
	...
	public function setSongTime(time:Float)
	{
		...
		vocals.pause();
		if (splitVocals) opponentVocals.pause();
		...
		if (splitVocals && Conductor.songPosition <= opponentVocals.length)
		{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			var playerVocals:openfl.media.Sound = Paths.voices(songData.song, (boyfriend.vocalsFile == null || boyfriend.vocalsFile.length < 1) ? 'Player' : boyfriend.vocalsFile, PlayState.SONG.props.vocalPrefix, PlayState.SONG.props.vocalSuffix);
			...
			try
        	{
				var oppVocals:openfl.media.Sound = Paths.voices(songData.song, (dad.vocalsFile == null || dad.vocalsFile.length < 1) ? 'Opponent' : dad.vocalsFile, PlayState.SONG.props.vocalPrefix, PlayState.SONG.props.vocalSuffix);
				if(oppVocals != null) 
				{
					trace('Opponent Vocals work!');
		...
		vocals.pitch = playbackRate;
		if (splitVocals) opponentVocals.pitch = playbackRate;
		FlxG.sound.list.add(vocals);
		if (splitVocals) FlxG.sound.list.add(opponentVocals);
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				vocals.stop();
				if (splitVocals) opponentVocals.stop();
				FlxG.sound.music.stop();
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
	{
		...
		vocals.volume = 0;
		if (splitVocals) opponentVocals.volume = 0;
		vocals.pause();
		if (splitVocals) opponentVocals.pause();
	...
	public function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.noteSplashes && ClientPrefs.noteSplashesType != 'Opponent' && note != null && !visualsOnlyMode) {
	...
	public function spawnNoteSplashOnOppNote(note:Note) {
		if(ClientPrefs.noteSplashes && ClientPrefs.noteSplashesType != 'Player' && ClientPrefs.opponentStrums && note != null && !visualsOnlyMode) {
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		switch (ClientPrefs.noteSkin) //Aqui pones los offsets EXTRA
		{
			case 'Vanilla':
	...
	{
		super.stepHit();
		if (SONG.needsVoices && FlxG.sound.music.time >= -Conductor.offset)
		{
			if (Math.abs(FlxG.sound.music.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)
				|| Math.abs(vocals.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)
				|| (splitVocals && Math.abs(opponentVocals.time - (Conductor.songPosition - Conductor.offset)) > (20 * playbackRate)))
			{
				resyncVocals();
			}
		}
