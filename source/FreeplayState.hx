...
class FreeplayState extends MusicBeatState
{
	...
	function getFromCharacter(name:String):Character.CharacterFile
	{
		...
		#end
		try
			return cast haxe.Json.parse(#if MODS_ALLOWED File.getContent(character) #else Assets.getText(character) #end)
	...
	override function update(elapsed:Float)
	{
		...
		else if(space && songs[curSelected].songName != "" && songs[curSelected].songName != "Friendship v2" && songs[curSelected].songName != "Friendship-v2")
		{
			...
			if(currentProps != propCheck)
			{
				...
				final P1:Character.CharacterFile = getFromCharacter(PlayState.SONG.player1);
				final P2:Character.CharacterFile = getFromCharacter(PlayState.SONG.player2);
				if (PlayState.SONG.needsVoices)
				{
					vocals = new FlxSound();
					opponentVocals = new FlxSound();
					final songName:String = songs[curSelected].songName.toLowerCase();
					final playerVocals:openfl.media.Sound = Paths.voices(songName, (P1.vocals_file == null || P1.vocals_file.length < 1) ? 'Player' : P1.vocals_file, PlayState.SONG.props.vocalPrefix, PlayState.SONG.props.vocalSuffix);
					final normalVocals:openfl.media.Sound = Paths.voices(songName, null, PlayState.SONG.props.vocalPrefix, PlayState.SONG.props.vocalSuffix);
					...
					opponentVocals.loadEmbedded(Paths.voices(songName, (P2.vocals_file == null || P2.vocals_file.length < 1) ? 'Opponent' : P2.vocals_file, PlayState.SONG.props.vocalPrefix, PlayState.SONG.props.vocalSuffix));
				}
				else {
					vocals = new FlxSound();
					opponentVocals = new FlxSound();
				...
				if (opponentVocals != null && opponentVocals.length > 0) FlxG.sound.list.add(opponentVocals);
				FlxG.sound.list.add(vocals);
				FlxG.sound.playMusic(Paths.inst(PlayState.SONG.song, PlayState.SONG.props.instPrefix, PlayState.SONG.props.instSuffix), 0.7);
				vocals.play();
				vocals.persist = true;
				vocals.looped = true;
				vocals.volume = 0.7;
