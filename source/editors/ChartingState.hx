...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Set Character Idle Speed', "Sets BF, GF or DAD head bopping speed.\n\nValue 1: The character whose idle speed you want to\nchange. Must be 'bf', 'gf' or 'dad', then type 'true' or\n'false' if the character must do the 'dance' animation\nin the opposite direction (useful only when 'danceLeft'\nand 'danceRight' exist), by default is 'false'.\n(Example: \"gf, true\", \"bf\").\n\nValue 2: The new value in beats at which you want\nthe character to play its idle animation. You can also\ntype '0.5' if you want the idle to play every half beat.\n\nWARNING: The value in beats must be a number related\nto the power of 2, that is 1, 2, 4, 8, 16, 32, etc. (with\nthe exception of '0.5'), otherwise the event will be\ncompletely ignored."],
		...
		['Invert Scroll Direction', "Kinda self explanatory, isn't it?"],
		...
		['Bad Battle Countdown', "Value 1: The value for the Bad Battle Countdown,\neither '3' or 'three', '2' or 'two', '1' or 'one', or\n'0' or 'go'."],
		...
		['Change Stage', "Changes the zRamírez's stage to the new or the old\none, depending on the value specified in \"Value 1.\"\n\nSet Value 1 to \"new\" or \"0\" to display the\nnew stage.\nSet Value 1 to \"old\" or \"1\" to display the\nold stage.\n\nThis event only works if the \"Stage-Rami-Changer\"\nis the current stage in any song."],
	...
	**/
	...
	override function create()
	{
		...
		else
		{
			...
			_song = {
				...
				needsVoices: true,
				arrowSkin: '',
				splashSkin: 'noteSplashesVanilla',//idk it would crash if i didn't
				player1: 'bf',
				player2: 'dad',
				gfVersion: 'gf',
				speed: 1,
	...
	function addSectionUI():Void
	{
		...
		check_altAnimBF = new FlxUICheckBox(check_altAnim.x, check_mustHitSection.y, null, null, "Alt Animation BF", 100);
		check_altAnimBF.checked = _song.notes[curSec].altAnimBF;
		...
		check_notesSec = new FlxUICheckBox(15, clearSectionButton.y + 25, null, null, "Notes", 100);
		check_notesSec.checked = true;
		check_eventsSec = new FlxUICheckBox(check_notesSec.x + 110, check_notesSec.y, null, null, "Events", 100);
	...
	function addChartingUI() {
		...
		waveformUseOpponentVoices = new FlxUICheckBox(waveformUseInstrumental.x + 200, waveformUseInstrumental.y, null, null, "Waveform\n(Opp. Vocals)", 100);
		...
		check_mute_inst = new FlxUICheckBox(10, 280, null, null, "Mute Instrumental\n(in editor)", 100);
		...
		check_warnings = new FlxUICheckBox(10, 125, null, null, "Ignore Progress Warnings", 100);
		...
		var check_mute_vocals = new FlxUICheckBox(check_mute_inst.x, check_mute_inst.y + 30, null, null, "Mute Main Vocals\n(in editor)", 100);
		...
		var check_mute_opponent_vocals = new FlxUICheckBox(check_mute_vocals.x + 120, check_mute_vocals.y, null, null, "Mute Opp. Vocals\n(in editor)", 100);
		check_mute_opponent_vocals.checked = false;
		check_mute_opponent_vocals.callback = function()
		{
			if(opponentVocals == null) return;
			opponentVocals.volume = check_mute_opponent_vocals.checked ? 0 : 1;
		};
		...
		instVolume = new FlxUINumericStepper(metronomeStepper.x, 240, 0.1, 1, 0, 1, 1);
		...
		opponentVoicesVolume = new FlxUINumericStepper(instVolume.x + 200, instVolume.y, 0.1, 1, 0, 1, 1);
		...
		tab_group_chart.add(new FlxText(opponentVoicesVolume.x, opponentVoicesVolume.y - 15, 0, 'Opp. Vocals'));
		...
		tab_group_chart.add(check_mute_opponent_vocals);
	...
	function getFromCharacter(name:String):Character.CharacterFile
	{
		...
		if (!FileSystem.exists(character)) return null;
		#else
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case "Alt Animation BF":
					_song.notes[curSec].altAnimBF = check.checked;
			}
		}
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'opponent_voices_volume')
			{
